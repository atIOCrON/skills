...
- Optional record file of `branch old_remote_sha rebased_head_sha` triples,
  one whitespace-separated triple per line, as written by the merge-stack
  workflow. When no record file is supplied, no branch ref may be advanced and
  only branches whose heads are already ancestors of the target branch may be
  deleted. Standalone mode sweeps every such merged local branch, including
  long-lived ones; supply a record file when cleanup should touch only the
  just-merged stack.
